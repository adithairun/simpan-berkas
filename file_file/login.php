 ...
 *
 *   $Id: login.php, v1.28 04/11/2018 21:08 Flava Clown $
 *
 *
 ***************************************************************************/
...
//----------------------------------------------------------------------------
	...
	if (preg_match("/^[a-z0-9$extra_character][a-z0-9$extra_character _-]{0,$un_lenght}[a-z0-9]$/i", $user_name))
	{
		...
		if (file_exists($userfilename))
		{
			...
			if (check_user_password($user_name, $user_pass))
			{
				...
				{
					if (preg_match('/^m:/', $user_temp_info))
...
case 'customizeprofile':
	...
	if ($logged_user_name != '')
	{
		if (check_is_user_session_active($logged_user_name))
		{
			...
			{
				if (preg_match( "/^([a-z0-9_]|\\-|\\.)+@(([a-z0-9_]|\\-)+\\.)+[a-z]{2,4}$/i", $typed_email))
				{
