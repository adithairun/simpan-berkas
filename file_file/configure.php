 ...
 *
 *   $Id: configure.php, v1.27 04/11/2018 22:09 Flava Clown $
 *
 *
 ***************************************************************************/
...
function print_configure_page()
{
	...
	{
		if(preg_match("/^.*\.txt$|^.*\.htm$|^.*\.html$/i",$filename))
		{
			if (!is_dir("$cfg_folder_name/$filename") && !preg_match($reserved_files, $filename))
			{
				echo"
					<tr>
	...
	<tr>
	  <td align=\"center\" bgColor=\"$tablecolor\">
	    <form name=\"".ACTION_EDITFILE."\" action=\"configure.$phpExt?".SID."\" method=\"post\" style=\"margin: 0\">
		  ...
		  <br><label for=\"password\">$mess[83]:</label><input type=\"password\" name=\"password\" id=\"password\"> 
	      <br><input type=\"submit\" class=\"vform2\" value=\"$mess[168]\">
	    </form>
	  </td>
	</tr>
	</table>";
...
switch($action)
{
	...
	case ACTION_EDITFILE;
		...
		$filename = basename(getGetVar('filename'));
	...
	case ACTION_SAVEFILE;
		...
		$filebody = str_replace(array("&lt;", "&gt;"), array("<", ">"), htmlentities($filebody, ENT_NOQUOTES, 'UTF-8', FALSE));
